function rotateImage(a) {
    ...
    for(let i = 0; i < Math.ceil(len / 2); i++){
        for( let j = 0;j < Math.floor(len / 2); j++){
            ...
            let temp = a[i][j];
            a[i][j] = a[revJ][i];
            a[revJ][i] = a[revI][revJ];
            a[revI][revJ] = a[j][revI];
            a[j][revI] = temp;
        }
    }
    return a;
...
function deepComp(array1, array2){
...
console.log(deepComp(rotateImage(a), aAnswer));
console.log(deepComp(rotateImage(b), bAnswer));
console.log(deepComp(rotateImage(c), cAnswer));
console.log(deepComp(rotateImage(d), dAnswer));
console.log(deepComp(rotateImage(e), eAnswer));
